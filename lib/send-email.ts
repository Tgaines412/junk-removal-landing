...
export async function sendQuoteEmail(formData: FormData) {
  console.log("Server action called - sendQuoteEmail")
  ...
  try {
    ...
    console.log("Form data extracted:", { name, phone, email, address, hasPhotos: photos.length > 0, photoCount: photos.length })
      ...
      <html>
        ...
        <body>
          <div class="container">
            ...
            <div class="content">
              <div class="urgent">
                <strong>⏰ URGENT:</strong> Customer expects response within 1 hour as promised on website!
              </div>
              ...
              <div class="photo-info">
                ...
                `).join('')}
                <p><em>Photo attachments will be processed separately</em></p>
              </div>
              `
                  : `
              <div class="photo-info">
    ...
    // Plain text version
    const textContent = `
NEW QUOTE REQUEST - BIN BUSTERS
===============================

⏰ URGENT: Customer expects response within 1 hour!
...
Photos: ${photos.length > 0 ? `${photos.length} photo${photos.length > 1 ? 's' : ''} uploaded` : "No photos uploaded"}

Please respond to this customer within 1 hour as promised on the website.

---
This request was submitted through the Bin Busters website.
    `
    ...
    const emailOptions: any = {
      from: "Bin Busters <onboarding@resend.dev>", // Using Resend's default domain for testing
    ...
    console.error("Unexpected error in sendQuoteEmail:", error)
    return {
      success: false,
      message: "Sorry, there was an error sending your request. Please call us directly at 021 448 097.",
    }
